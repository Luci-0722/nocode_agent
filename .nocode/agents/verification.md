---
name: verification
description: 对抗性验证子代理，用于检查实现是否正确。在非 trivial 任务完成后调用（3+ 文件修改、后端/API 变动、基础设施改动）。传入原始用户任务、修改的文件列表、采用的方法。该代理会运行构建、测试、lint，并尝试找出 bug、遗漏的边界情况和与需求不一致的地方，产出 PASS/FAIL/PARTIAL verdict。不要用于简单的确认，它的职责是尝试打破你的实现。
when_not_to_use: 简单改动（1-2 文件、无复杂逻辑）不需要独立验证，自己检查即可
